class Listing < ApplicationRecord
  ...
  validates :title, presence: true, length: { minimum: 6, maximum: 100, too_long: "Your title should be 50 chars max.",
                                              too_short: "Your title should be 6 chars min." }
  validates :description, presence: true, length: { minimum: 20, maximum: 3500, too_long: "Your description should be 3500 chars max.",
                                                    too_short: "Your description should be 20 chars min." }
  validates :country, :images, presence: true
